...
![alt](images/shop - shop - public.png)

**Теперь запросы:**
1. Вывести выручку по месяцам:
```sql
SELECT 
...
3. Количество заказов по статусам (completed, cancelled, processing)
```sql
--Всего заказов
...
8. ABC-анализ товаров:<br>
Логика правильного ABC-анализа<br>
 1. Рассчитать общую выручку.

 2. Рассчитать выручку на каждый товар.

 3. Отсортировать по выручке и посчитать долю выручки от общей выручки.

 4. Посчитать накопительную (кумулятивную) долю — то есть сколько процентов от общей выручки приносят все товары, начиная с самого первого и заканчивая текущим.

 5. Разбить на три группы: A - группа товаров с самой большой выручкой с накопительной до 80%<br>
...
<details><summary>Выручка по месяцам</summary>
'''python
# Выручка по месяцам
import pandas as pd
...
<details><summary>Количество заказов по месяцам</summary>
'''python
import pandas as pd
import matplotlib.pyplot as plt
from sqlalchemy import create_engine
